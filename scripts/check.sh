...
MISSING_NEWLINES=$(find . -type f \( -name "*.py" -o -name "*.md" -o -name "*.toml" -o -name "*.yml" -o -name "*.yaml" -o -name "*.sh" -o -name ".gitignore" -o -name "*.txt" -o -name "*.json" -o -name "*.html" -o -name "*.css" \) -not -path "./.venv/*" -not -path "./.git/*" -not -path "./.pytest_cache/*" -not -path "./.ruff_cache/*" -not -path "./.mypy_cache/*" | while read -r file; do
